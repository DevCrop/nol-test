...
// 라우트 시스템이 있으면 라우트로도 확인
if (function_exists('current_route_is')) {
    if (current_route_is('home')) {
        $isMainPage = true;
    }
}
...
if ($isMainPage) {
    ...
    $stmt = $db->query("SELECT * FROM nb_counter_config LIMIT 1");
    $counter_config_row = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($counter_config_row) {
        $Counter_ON = "N";
    } else {
    }
    ...
    if ($counter_config_row) {
        ...
        if ($Counter_ON == "Y" && !$isRobot && $counter_config_row['Counter_Use'] == "Y") {
